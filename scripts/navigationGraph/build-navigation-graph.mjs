...
// Загружаем GeoJSON
const filename = path.resolve(__dirname, '../overpass/tashkent_roads.geojson');
...
fs.writeFileSync(
  path.resolve(__dirname, 'dist/navigation-graph.json'),
  JSON.stringify(graph, null, 2),
);
console.log(
  `Граф сохранён в: ${path.resolve(__dirname, 'dist/navigation-graph.json')})`,
);

// Сохраняем компактную версию (без форматирования)
const compactFilename = path.resolve(
  __dirname,
  'dist/navigation-graph-compact.json',
);
fs.writeFileSync(compactFilename, JSON.stringify(graph));
console.log(`Компактная версия сохранена в: ${compactFilename}`);

// Статистика
const graphSize = fs.statSync(compactFilename).size;
