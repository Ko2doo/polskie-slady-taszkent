...
const filename = nodePath.resolve(__dirname, 'dist/navigation-graph-compact.json');
...
// Сохраняем
const optimizedFilename = nodePath.resolve(__dirname, 'dist/optimized/navigation-graph-optimized.json');

fs.mkdirSync(nodePath.resolve(__dirname, 'dist/optimized'), { recursive: true });
fs.writeFileSync(optimizedFilename, JSON.stringify(optimizedGraph));

const originalSize = fs.statSync(filename).size;
const optimizedSize = fs.statSync(optimizedFilename).size;
